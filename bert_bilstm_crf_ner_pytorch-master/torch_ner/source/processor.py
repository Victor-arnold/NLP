...
class NerProcessor(object):
    ...
    def get_dataset(self, config: Config, tokenizer, mode="train"):
        ...
        # 读取输入数据，进一步封装 example[0]: 'label':'O B-ENT ...', 'text': '《 机 械 设 计...'
        examples = self.get_input_examples(file_path, separator=config.sep)

        # 对输入数据进行特征转换 input_ids, token_type_ids, attention_mask, label_id, label_id
        features = self.convert_examples_to_features(config, examples, tokenizer)

        # 获取全部数据的特征，封装成TensorDataset
        all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long) # torch.Size: (3, 128)
        all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long) # torch.Size: (3, 128)
        all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long) #  torch.Size: (3, 128)
        all_label_ids = torch.tensor([f.label_id for f in features], dtype=torch.long)
        # data: 3 * 4 , 每一个data[i] i<=2， 都是一个长度为4的，包含一下四个。相当于四个每个抽取一个构成
